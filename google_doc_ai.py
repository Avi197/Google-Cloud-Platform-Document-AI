...
def deskew_pdf(in_path, out_path, temp_path="./temp/temp_deskew"):
    ...
    for index, page in enumerate(pages):
        print(f"Process for page {index}")
...
def check_new_file(in_path, out_path):
    ...
    out_list = [i.replace(f'{out_path}/', '').replace('.xls', '') for i in out_li[1:]]
...
if __name__ == '__main__':
    ...
    # - deskewed_out: json output from doc ai
    ...
    deskewed_json_path_bucket_full = f"gs://{bucket_name}/gcp_process/deskewed_out"
    deskewed_json_path_bucket = "gcp_process/deskewed_out"
    ...
    while True:
        ...
        list_new_file = check_new_file(in_pdf_bucket, xlsx_path_bucket)
        ...
        if list_new_file:
            ...
            for (file_name_pdf, pdf_file_path, deskewed_file_path) in file_paths:
                # xlsx_name = file_name_pdf.replace('pdf', 'xls')
                ...
                if not check_blob_exists(bucket_name, f'{deskewed_path_bucket}/{file_name_pdf}'):
                    ...
                    file_name_raw = file_name_pdf.replace('.pdf', '')
                    upload_blob(bucket_name, deskewed_file_path, f"{deskewed_path_bucket}/{file_name_raw}")
                ...
                # if not check_blob_exists(bucket_name, f'{deskewed_json_path_bucket}/{file_name}/'):
                print("start document ai process")
                start_time_doc_ai = timer()
                blob_list = get_document_ai_batch_result_json(deskewed_path_bucket, deskewed_json_path_bucket,
                                                              f'{file_name_pdf}')
                end_time_doc_ai = timer()
                print(f"Doc AI took {end_time_doc_ai - start_time_doc_ai} seconds to complete")
                print("----------------------------------------------------------------------------")

                # process and write to xlsx
                # ---------------------------------------------------------------------------------------
                print("start writing output to xlsx file")

                extract_tables, extract_form_fields = get_extract_data_from_blob(blob_list)

                merge_index = get_merge_index(extract_tables)
                update_tables = update_merge_table(extract_tables, merge_index)
                update_tables = remove_duplicate_header(update_tables)
                # write xlsx result to VM

                write_table_form_to_xlsx(update_tables, extract_form_fields, f'{xlsx_path_local}/{xlsx_name}')
                print("----------------------------------------------------------------------------")

                # ---------------------------------------------------------------------------------------

                # upload xlsx file to bucket
                upload_blob(bucket_name, f'{xlsx_path_local}/{xlsx_name}', f'{xlsx_path_bucket}/{xlsx_name}')

                # remove temp xlsx file
                filelist = glob(f"{xlsx_path_local}/*")
                for f in filelist:
                    os.remove(f)
